...
public class ExecuteActionsAndTerminateTask extends ExecuteActionsBaseEntry {
    ...
    public Serializable execute(final Map<String, Serializable> parameters, final TenantServiceAccessor tenantAccessor)
        ...
        try {
            ...
            try {
                ...
                updateActivityInstanceVariables(operations, operationsContext, sActivityInstanceID,
                        processDefinitionID);
            } finally {
    ...
    protected Long getActivityInstanceId(final Map<String, Serializable> parameters)
            throws SCommandParameterizationException {
        final String message = "Mandatory parameter " + ACTIVITY_INSTANCE_ID_KEY
                + " is missing or not convertible to long.";
        return getMandatoryParameter(parameters, ACTIVITY_INSTANCE_ID_KEY, message);
    }

    protected List<Operation> getOperations(final Map<String, Serializable> parameters)
            throws SCommandParameterizationException {
    ...
    protected Map<String, Serializable> getOperationsContext(final Map<String, Serializable> parameters)
            throws SCommandParameterizationException {
    ...
    protected void updateActivityInstanceVariables(final List<Operation> operations,
            final Map<String, Serializable> operationsContext,
            final long activityInstanceId, final Long processDefinitionID) throws SOperationExecutionException {
        SExpressionContext sExpressionContext = buildExpressionContext(operationsContext, activityInstanceId,
                processDefinitionID);
        List<SOperation> sOperations = ServerModelConvertor.convertOperations(operations);
        getOperationService().execute(sOperations, activityInstanceId, DataInstanceContainer.ACTIVITY_INSTANCE.name(),
                sExpressionContext);
    }

    private SExpressionContext buildExpressionContext(final Map<String, Serializable> operationsContext,
    ...
    protected void executeActivity(final SFlowNodeInstance flowNodeInstance, long executerUserId)
        ...
        processExecutor.executeFlowNode(flowNodeInstance, executerUserId, executerSubstituteId);
        if (logger.isLoggable(getClass(), TechnicalLogSeverity.INFO) && flowNodeInstance.getStateId() != 0 /*
                                                                                                            * don't log
                                                                                                            * when
                                                                                                            * create
                                                                                                            * subtask
            ...
            final String message = LogMessageBuilder.buildExecuteTaskContextMessage(flowNodeInstance,
                    sessionInfos.getUsername(), executerUserId,
    ...
    protected void addSystemCommentOnProcessInstanceWhenExecutingTaskFor(final SFlowNodeInstance flowNodeInstance,
