...
import org.bonitasoft.engine.bpm.model.CallActivityDefinition;
...
import org.bonitasoft.engine.bpm.model.FlowNodeDefinition;
import org.bonitasoft.engine.bpm.model.GatewayDefinition;
import org.bonitasoft.engine.bpm.model.HumanTaskDefinition;
...
import org.bonitasoft.engine.bpm.model.UserFilterDefinition;
import org.bonitasoft.engine.bpm.model.UserTaskDefinition;
import org.bonitasoft.engine.bpm.model.data.DataDefinition;
import org.bonitasoft.engine.bpm.model.data.XMLDataDefinition;
import org.bonitasoft.engine.bpm.model.event.BoundaryEventDefinition;
import org.bonitasoft.engine.bpm.model.event.CatchEventDefinition;
import org.bonitasoft.engine.bpm.model.event.EndEventDefinition;
import org.bonitasoft.engine.bpm.model.event.IntermediateCatchEventDefinition;
import org.bonitasoft.engine.bpm.model.event.IntermediateThrowEventDefinition;
import org.bonitasoft.engine.bpm.model.event.StartEventDefinition;
import org.bonitasoft.engine.bpm.model.event.ThrowEventDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.CatchErrorEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.CatchMessageEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.CatchSignalEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.CorrelationDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.MessageEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.TerminateEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.ThrowErrorEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.ThrowMessageEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.ThrowSignalEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.event.trigger.TimerEventTriggerDefinition;
import org.bonitasoft.engine.bpm.model.impl.SubProcessDefinitionImpl;
...
public class XMLProcessDefinition {
    ...
    public static final String RIGHT_OPERAND_VARIABLE = "dataName";
    ...
    private void createAndfillMessageEventTriggers(final XMLNode eventNode, final ThrowEventDefinition event) {
        for (final ThrowMessageEventTriggerDefinition messageEventTrigger : event.getMessageEventTriggerDefinitions()) {
            ...
            addExpressionNode(messageEventTriggerNode, TARGET_FLOW_NODE, messageEventTrigger.getTargetFlowNode());
            createAndFillDataDefinitions(messageEventTriggerNode, messageEventTrigger);
            createAndfillCorrelations(messageEventTriggerNode, messageEventTrigger);
    ...
    private void createAndfillSignalEventTriggers(final XMLNode catchEventNode, final CatchEventDefinition catchEvent) {
        for (final CatchSignalEventTriggerDefinition signalEventTrigger : catchEvent.getSignalEventTriggerDefinitons()) {
    ...
    private void createAndfillMessageEventTriggers(final XMLNode catchEventNode, final CatchEventDefinition catchEvent) {
        for (final CatchMessageEventTriggerDefinition messageEventTrigger : catchEvent.getMessageEventTriggerDefinitons()) {
            final XMLNode messageEventTriggerNode = new XMLNode(CATCH_MESSAGE_EVENT_TRIGGER_NODE);
            messageEventTriggerNode.addAttribute(NAME, messageEventTrigger.getMessageName());
            createAndfillCorrelations(messageEventTriggerNode, messageEventTrigger);
            createAndfillOperations(messageEventTriggerNode, messageEventTrigger.getOperations(), OPERATION_NODE);
            catchEventNode.addChild(messageEventTriggerNode);
        }
    }

    private void createAndfillCorrelations(final XMLNode messageEventTriggerNode, final MessageEventTriggerDefinition messageEventTrigger) {
    ...
    private void createAndFillOperation(final XMLNode parentNode, final Operation operation, final String operationNodeName) {
        ...
        fillLeftOperandNode(rightOperand, operation.getVariableToSet());
    ...
    private void fillLeftOperandNode(final XMLNode rightOperandNode, final LeftOperand leftOperand) {
        rightOperandNode.addAttribute(RIGHT_OPERAND_VARIABLE, leftOperand.getDataName());
    }
    ...
    private void fillDocumentDefinitionNode(final XMLNode documentDefinitionNode, final DocumentDefinition documentDefinition) {
        ...
        documentDefinitionNode.addAttribute(DOCUMENT_DEFINITION_FILE_NAME, documentDefinition.getFileName());
    ...
    private void fillTransitionNode(final XMLNode transitionNode, final TransitionDefinition transition) {
        ...
        transitionNode.addAttribute(TRANSITION_SOURCE, transition.getSource());
        transitionNode.addAttribute(TRANSITION_TARGET, transition.getTarget());
