...
public class ArchivedUserTaskContextResource extends CommonResource {
    ...
    @Get("json")
    public Map<String, Serializable> getArchivedUserTaskContext()
            throws UserTaskNotFoundException, ExpressionEvaluationException {
        final Map<String, Serializable> resultMap = new HashMap<>();

        Map<String, Serializable> archivedUserTaskExecutionContext = processAPI
                .getArchivedUserTaskExecutionContext(getArchivedTaskIdParameter());
    ...
    protected long getArchivedTaskIdParameter() {
        ...
        if (taskId == null) {
            throw new APIException(
                    "Attribute '" + ARCHIVED_TASK_ID + "' is mandatory in order to get the archived task context");
        }
