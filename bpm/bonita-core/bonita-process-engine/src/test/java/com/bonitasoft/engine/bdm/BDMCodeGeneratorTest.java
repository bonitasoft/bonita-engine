...
public class BDMCodeGeneratorTest {
	...
	private BDMCodeGenerator codeGenerator;
	...
	public void setUp() throws Exception {
		codeGenerator = new BDMCodeGenerator(new BusinessObjectModel());
	}
	...
	public void shouldbuildAstFromBom_FillModel() throws Exception {
		...
		employeeBO.setClassName("Employee");
		bom.addBusinessObject(employeeBO);
		codeGenerator = new BDMCodeGenerator(bom);
		codeGenerator.buildASTFromBom();
		assertThat(codeGenerator.getModel()._getClass("Employee")).isNotNull();
	}
	
	@Test
	public void shouldAddEntity_CreateAValidEnityFromBusinessObject() throws Exception {
		BusinessObject employeeBO = new BusinessObject();
		employeeBO.setClassName("org.bonitasoft.hr.Employee");
		codeGenerator.addEntity(employeeBO);
		JDefinedClass definedClass = codeGenerator.getModel()._getClass(employeeBO.getClassName());
	...
	public void shouldAddFieldAndAccessors_CreatePrimitiveAttribute_AndAccessorMethods_InDefinedClass() throws Exception {
		...
		JDefinedClass definedClass = codeGenerator.addClass("org.bonitasoft.hr.Employee");
		codeGenerator.addFieldAndAccessors(definedClass, nameField);
	
		assertThat(definedClass.fields().get("name")).isNotNull();
		assertThat(definedClass.fields().get("name").type()).isEqualTo(codeGenerator.getModel().ref(String.class.getName()));
