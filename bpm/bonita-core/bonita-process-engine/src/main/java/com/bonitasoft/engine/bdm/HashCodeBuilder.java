	...
	public JMethod generate(JDefinedClass definedClass) {
		...
		for(Entry<String, JFieldVar> field : definedClass.fields().entrySet()){
			...
			if(!unboxifyType.isPrimitive()){
				JVar refHashCode = body.decl(JType.parse(definedClass.owner(), int.class.getName()), fieldVar.name()+"Code",JExpr.lit(0));
				...
				if(unboxifyType.name().equals(boolean.class.getSimpleName())){
					JVar refHashCode = body.decl(JType.parse(definedClass.owner(), int.class.getName()),  fieldVar.name()+"Code",JExpr.lit(1237));
					...
					JVar temp = body.decl(JType.parse(definedClass.owner(), long.class.getName()),  fieldVar.name()+"temp",doubleType.staticInvoke("doubleToLongBits").arg(fieldVar));
