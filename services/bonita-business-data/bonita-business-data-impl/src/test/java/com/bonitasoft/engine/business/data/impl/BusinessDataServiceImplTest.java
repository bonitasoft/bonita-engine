...
public class BusinessDataServiceImplTest {
    ...
    public void callJavaOperationShouldWithProxyfiedEntityShouldUsedRealEntityClass() throws Exception {
        ...
        doReturn(entity).when(businessDataRepository).findById(entity.getClass(), entity.getPersistenceId());
        doReturn(entity).when(businessDataRepository).merge(entity);

        //when
        final EntityPojo pojoObject = (EntityPojo) businessDataService.callJavaOperation(pojo, proxyfiedEntity, "setAggregationEntity",
                Entity.class.getName());
