...
public class CSVOneFlushEventListener implements FlushEventListener {
    ...
    /**
     * keep the Csv Separator (in general, ; is expected)
     */
    private final String csvSeparator;

    /**
     * name of the folder on the disk to write the file
     */
    ...
    public FlushResult flush(final FlushEvent flushEvent) throws Exception {
        ...
        final boolean exist = outputFile.exists();
        ...
        if (!exist)
        {
            // header
            boolean first = true;
            for (final String value : getHeaderRow())
            {
                if (!first) {
        ...
        for (final Record record : records) {
            boolean first = true;
            for (final String value : getRow(record))
            {
                if (!first) {
                    writer.append(csvSeparator);
                }
